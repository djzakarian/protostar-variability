...
for obj in obj_names:
    for sub_dir in new_directory_list:
        path = (f'{coadd_directory}/{obj}/{sub_dir}')
        paths.append(path)
        for band in bands:
            path = (f'{coadd_directory}/{obj}/{sub_dir}/{band}')
    
for filename in os.listdir(coadd_directory):
    if search_str in filename:
        new_filename = filename.replace(search_str, "")
        original_path = os.path.join(coadd_directory, filename)
        new_path = os.path.join(coadd_directory, new_filename)
        os.rename(original_path, new_path)
