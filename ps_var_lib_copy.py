...
def time_series_tab(processed_images_dir, epochs_tab_path, wcs_apertures, wcs_annuli):
    ...
    for file in file_list_paths:
        ...
        pattern = 'size0_0667_ep(\d+)_b1ICORE'
        match = re.search(pattern, file)
        obj_epoch = int(match.group(1))
...
def plot_compstars(time_series_tab, style='seaborn-dark'):
...
def combine_images(processed_images_dir):
    ...
    coadd_hdu.writeto(f'{processed_images_dir}/coadd.fits', overwrite=True)
...
#%% subtract_coadd
def subtract_coadd(processed_images_dir, mask_sigma_coeff = 5):
